...
class BacteriaDatabase:
    ...
    def get_all_record_ids(self):
        #write the query to get all unique record_id from the gff table
    ...
    def get_protein_ids_from_record_id(self, record_id):
        #write function to return list of protein_ids for a given record_id
        query = f"""SELECT * FROM gff WHERE record_id = '{record_id}'"""
        df = self.query(query)
